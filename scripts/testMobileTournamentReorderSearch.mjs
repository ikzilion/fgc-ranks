...
// x-position order are pixel-identical to before, and confirms the search
// input actually filters the rendered entrant list.
//
...
async function main() {
  ...
  try {
    ...
    // --- Functional filter check (still at a mobile width) ---
    ...
    assert(countBefore === entrantTags.length, `All ${entrantTags.length} entrants render before filtering (found ${countBefore})`);
    ...
    const remainingText = countAfter > 0 ? await entrantLinks.first().innerText() : "";
    assert(
      countAfter === 1 && remainingText.includes("ZZZUniqueSearchableTag123"),
      `Search filters entrant list to only the matching tag (count=${countAfter}, text="${remainingText.split("\n")[0]}")`
    );
